...
TRADUCTOR = defaultdict(lambda:"Nombre",{
    "JJ": "Adjetivo"  ,   
	"UH": "Interjeccion"  , 
	"VBG": "Verbo", 
	"NN": "Nombre"  , 
...
def wikicorpus(words=1000000, start=0):
    s = [[]]
    i = 0
    for f in glob("/media/yoshi/Data/Proyectos/tagged.es/*")[start:]:
...
def lexicon_function():
    ...
    for sentence in wikicorpus(12000000):
    ...
    open("es-lexicon4.txt", "w").write(BOM_UTF8 + "\n".join(top).encode("utf-8"))
    
def rules_function():
    sentences = wikicorpus(words=8000000)
    ...
    open("es-context3.txt", "w").write(BOM_UTF8 + "\n".join(ctx).encode("utf-8"))
...
def main():
    ...
    parser = SpanishParser(morphology_path="es-morphology2.txt",
                           ...
                           lexicon_path= "es-lexicon4.txt")

    """
    cadena = parser.parse(u"me cago en los malditos judios de mierda",tokenize=True)
